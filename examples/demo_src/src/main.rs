...
fn handle_all_data_option(src: &mut SacnSource, split_input: Vec<&str>) -> Result<bool> {
    if split_input.len() < 3 {
        return Err(std::io::Error::new(std::io::ErrorKind::InvalidInput, "Insufficient parts for data line ( < 3 )"));
    }
...
fn handle_unicast_option(src: &mut SacnSource, split_input: Vec<&str>) -> Result<bool> {
    ...
    if split_input.len() < 5 {
        return Err(std::io::Error::new(std::io::ErrorKind::InvalidInput, "Insufficient parts for data line ( < 5 )"));
    }
...
fn handle_test_preset_option(src: &mut SacnSource, split_input: Vec<&str>) -> Result<bool> {
    if split_input.len() < 3 {
        return Err(std::io::Error::new(std::io::ErrorKind::InvalidInput, "Insufficient parts for test preset option ( < 3 )"));
    }
    ...
    match preset {
        ...
        TEST_PRESET_TWO_UNIVERSE => {
            if split_input.len() < 4 {
                return Err(std::io::Error::new(std::io::ErrorKind::InvalidInput, "Insufficient parts for test preset 2 universes option ( < 4 )"));
            }
        ...
        _ => {
            return Err(std::io::Error::new(std::io::ErrorKind::InvalidInput, "Unrecognised test preset option"));
        }
    }
            ...
            match split_input[0] {
                ...
                ACTION_PREVIEW_OPTION => {
                    ...
                    match val {
                        ...
                        Err(_e) => {
                            return Err(std::io::Error::new(std::io::ErrorKind::InvalidInput, "Preview flag option not 'true'/'false' or otherwise parsable as boolean"));
                        }
                    }
                    Ok(true)
                }
