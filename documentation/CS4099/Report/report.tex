...
test-> compliance -> conformance -> what it means for there to be 'sufficient' testing
	...
	The intended user for this library is a software developer developing applications that utilise the sACN protocol. It isn't designed to be used directly by an end user as it is just a library which needs to be used in code to actually perform any actions.	\\	
...
\subsection{SacnReceiver}
	...
	The decision not to pass up data packet awaiting synchronisation comes with an additional
	
	
	As data packets awaiting synchronisation are stored in a 
	
	
	As discussed above data-packets are only passed up by a receiver when they are ready, this means that there is an underlying data structure which stores the waiting data
	-> could get very large -> set a limit for embedded systems.
	...
	-- Std/No-Std means won't work on all embedded devices, also the threading required for source.
