...
Testing found a number of problems with existing implementation:
- No checking for the sync universe / universe / priority being in range

One problem found was that the option field wasn't handled correctly, with the library taking bit 7 to mean the 7th bit when it is specified in ANSI E1.31-2018 that the 7th bit means the most significant bit and the 0th bit is the least significant. Wire-shark also shows that the original implementation was incorrect as it also takes the most significant bit to be preview_data (bit 7) and so on. This wasn't found until the unit tests which check that each option is assigned correctly were performed, without these tests it might be difficult to isolate this problem as within the source code it was only a character's difference between the correct and wrong placement which is easily overlooked. This shows why thorough testing is a vital part of the process as developer 'dry-runs' or running a few high-level, shallow tests are unlikely to find problems such as this.
