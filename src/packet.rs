...
pub fn is_universe_in_range(universe: u16) -> Result<()> {
    ...
    {
        return Err(SacnError::IllegalUniverse(
            format!(
...
fn parse_source_name_str(buf: &[u8]) -> Result<&str> {
    ...
    if source_name_length == buf.len() && buf[buf.len() - 1] != 0 {
        return Err(SacnError::SacnParsePackError(
            sacn_parse_pack_error::SacnParsePackError::SourceNameInvalid(
        ...
        impl$( $lt )* AcnRootLayerProtocol$( $lt )* {
            ...
            pub fn parse(buf: &[u8]) -> Result<AcnRootLayerProtocol> {
                if buf.len() <  (E131_PREAMBLE_SIZE as usize) {
                    return Err(SacnError::SacnParsePackError(
            sacn_parse_pack_error::SacnParsePackError::ParseInsufficientData("Insufficient data for ACN root layer preamble".to_string())));
                }

                // Preamble Size
                if NetworkEndian::read_u16(&buf[0..2]) != E131_PREAMBLE_SIZE {
                    return Err(SacnError::SacnParsePackError(sacn_parse_pack_error::SacnParsePackError::ParseInvalidData("invalid Preamble Size".to_string())));
                }

                // Post-amble Size
                if NetworkEndian::read_u16(&buf[2..4]) != E131_POSTAMBLE_SIZE {
                    return Err(SacnError::SacnParsePackError(
            sacn_parse_pack_error::SacnParsePackError::ParseInvalidData("invalid Post-amble Size".to_string())));
                }

                // ACN Packet Identifier
                if &buf[4 .. (E131_PREAMBLE_SIZE as usize)] != E131_ACN_PACKET_IDENTIFIER {
                    return Err(SacnError::SacnParsePackError(
            sacn_parse_pack_error::SacnParsePackError::ParseInvalidData("invalid ACN packet identifier".to_string())));
                }
...
fn pdu_info(buf: &[u8], vector_length: usize) -> Result<PduInfo> {
    if buf.len() < E131_PDU_LENGTH_FLAGS_LENGTH + vector_length {
        return Err(SacnError::SacnParsePackError(
            sacn_parse_pack_error::SacnParsePackError::ParseInsufficientData(
    ...
    if flags != E131_PDU_FLAGS {
        return Err(SacnError::SacnParsePackError(
            sacn_parse_pack_error::SacnParsePackError::ParsePduInvalidFlags(flags)
        ));
    }
    // Length
    ...
    ( $( $lt:tt )* ) => {
        ...
        impl$( $lt )* Pdu for E131RootLayer$( $lt )* {
            fn parse(buf: &[u8]) -> Result<E131RootLayer$( $lt )*> {
                ...
                if buf.len() < length {
                    return Err(SacnError::SacnParsePackError(
            sacn_parse_pack_error::SacnParsePackError::ParseInsufficientData("Buffer contains insufficient data based on ACN root layer pdu length field".to_string())));
                }

                if vector != VECTOR_ROOT_E131_DATA && vector != VECTOR_ROOT_E131_EXTENDED {
                    return Err(SacnError::SacnParsePackError(
            sacn_parse_pack_error::SacnParsePackError::PduInvalidVector(vector)));
                }
                ...
                let data = match vector {
                    ...
                    VECTOR_ROOT_E131_EXTENDED => {
                        ...
                        if buf.len() < length {
                            return Err(SacnError::SacnParsePackError(
            sacn_parse_pack_error::SacnParsePackError::ParseInsufficientData("Buffer contains insufficient data based on E131 framing layer pdu length field".to_string())));
                        }
            ...
            fn pack(&self, buf: &mut [u8]) -> Result<()> {
                if buf.len() < self.len() {
                    return Err(SacnError::SacnParsePackError(
            sacn_parse_pack_error::SacnParsePackError::PackBufferInsufficient("".to_string())))
                }
    ...
    ( $( $lt:tt )* ) => {
        ...
        impl$( $lt )* Pdu for DataPacketFramingLayer$( $lt )* {
            fn parse(buf: &[u8]) -> Result<DataPacketFramingLayer$( $lt )*> {
                ...
                if buf.len() < length {
                    return Err(SacnError::SacnParsePackError(
            sacn_parse_pack_error::SacnParsePackError::ParseInsufficientData("Buffer contains insufficient data based on data packet framing layer pdu length field".to_string())));
                }
            ...
            sacn_parse_pack_error::SacnParsePackError::PduInvalidVector(vector)));
                ...
                if priority > E131_MAX_PRIORITY {
                    return Err(SacnError::SacnParsePackError(
                            sacn_parse_pack_error::SacnParsePackError::ParseInvalidPriority(
                ...
                if synchronization_address > E131_MAX_MULTICAST_UNIVERSE {
                    return Err(SacnError::SacnParsePackError(
                        sacn_parse_pack_error::SacnParsePackError::ParseInvalidSyncAddr(
                ...
                if universe < E131_MIN_MULTICAST_UNIVERSE || universe > E131_MAX_MULTICAST_UNIVERSE {
                    return Err(SacnError::SacnParsePackError(
                        sacn_parse_pack_error::SacnParsePackError::ParseInvalidUniverse(
    ...
    ( $( $lt:tt )* ) => {
        ...
        impl$( $lt )* Pdu for DataPacketDmpLayer$( $lt )* {
            ...
            fn parse(buf: &[u8]) -> Result<DataPacketDmpLayer$( $lt )*> {
                ...
                if buf.len() < length {
                    return Err(SacnError::SacnParsePackError(
            sacn_parse_pack_error::SacnParsePackError::ParseInsufficientData("Buffer contains insufficient data based on data packet dmp layer pdu length field".to_string())));
                }
            ...
            sacn_parse_pack_error::SacnParsePackError::PduInvalidVector(vector)));
                ...
                if buf[ADDRESS_DATA_FIELD_INDEX] != E131_DMP_LAYER_ADDRESS_DATA_FIELD {
                    return Err(SacnError::SacnParsePackError(
            sacn_parse_pack_error::SacnParsePackError::ParseInvalidData("invalid Address and Data Type".to_string())));
                }
                ...
                if NetworkEndian::read_u16(&buf[FIRST_PRIORITY_FIELD_INDEX .. ADDRESS_INCREMENT_FIELD_INDEX]) != E131_DATA_PACKET_DMP_LAYER_FIRST_PROPERTY_FIELD {
                    return Err(SacnError::SacnParsePackError(
            sacn_parse_pack_error::SacnParsePackError::ParseInvalidData("invalid First Property Address".to_string())));
                }
                ...
                if NetworkEndian::read_u16(&buf[ADDRESS_INCREMENT_FIELD_INDEX .. PROPERTY_VALUE_COUNT_FIELD_INDEX]) != E131_DATA_PACKET_DMP_LAYER_ADDRESS_INCREMENT {
                    return Err(SacnError::SacnParsePackError(
            sacn_parse_pack_error::SacnParsePackError::ParseInvalidData("invalid Address Increment".to_string())));
                }
                ...
                if property_values_length > UNIVERSE_CHANNEL_CAPACITY {
                    return Err(SacnError::SacnParsePackError(
            sacn_parse_pack_error::SacnParsePackError::ParseInvalidData("only 512 DMX slots allowed".to_string())));
                }
            ...
            fn pack(&self, buf: &mut [u8]) -> Result<()> {
                if self.property_values.len() > UNIVERSE_CHANNEL_CAPACITY {
                    return Err(SacnError::SacnParsePackError(
            sacn_parse_pack_error::SacnParsePackError::PackInvalidData("only 512 DMX values allowed".to_string())));
                }

                if buf.len() < self.len() {
                    return Err(SacnError::SacnParsePackError(
            sacn_parse_pack_error::SacnParsePackError::PackBufferInsufficient("DataPacketDmpLayer pack buffer length insufficient".to_string())));
                }
...
impl Pdu for SynchronizationPacketFramingLayer {
    fn parse(buf: &[u8]) -> Result<SynchronizationPacketFramingLayer> {
        ...
        if buf.len() < length {
            return Err(SacnError::SacnParsePackError(
            sacn_parse_pack_error::SacnParsePackError::ParseInsufficientData("Buffer contains insufficient data based on synchronisation packet framing layer pdu length field".to_string())));
        }
        ...
        if length != E131_UNIVERSE_SYNC_PACKET_FRAMING_LAYER_LENGTH {
            return Err(SacnError::SacnParsePackError(
                            sacn_parse_pack_error::SacnParsePackError::PduInvalidLength(length)
            ));
        }
    ...
    fn pack(&self, buf: &mut [u8]) -> Result<()> {
        if buf.len() < self.len() {
            return Err(SacnError::SacnParsePackError(
                            sacn_parse_pack_error::SacnParsePackError::PackBufferInsufficient(
    ...
    ( $( $lt:tt )* ) => {
        ...
        impl$( $lt )* Pdu for UniverseDiscoveryPacketFramingLayer$( $lt )* {
            fn parse(buf: &[u8]) -> Result<UniverseDiscoveryPacketFramingLayer$( $lt )*> {
                ...
                if buf.len() < length {
                    return Err(SacnError::SacnParsePackError(
            sacn_parse_pack_error::SacnParsePackError::ParseInsufficientData("Buffer contains insufficient data based on universe discovery packet framing layer pdu length field".to_string())));
                }
            ...
            fn pack(&self, buf: &mut [u8]) -> Result<()> {
                if buf.len() < self.len() {
                    return Err(SacnError::SacnParsePackError(
            sacn_parse_pack_error::SacnParsePackError::PackBufferInsufficient("UniverseDiscoveryPacketFramingLayer pack buffer length insufficient".to_string())));
                }
    ...
    ( $( $lt:tt )* ) => {
        ...
        impl$( $lt )* Pdu for UniverseDiscoveryPacketUniverseDiscoveryLayer$( $lt )* {
            ...
            sacn_parse_pack_error::SacnParsePackError::PduInvalidLength(length)));
                ...
                if page > last_page {
                    return Err(SacnError::SacnParsePackError(
            sacn_parse_pack_error::SacnParsePackError::ParseInvalidPage("Page value higher than last_page".to_string())));
                }
            ...
            fn pack(&self, buf: &mut [u8]) -> Result<()> {
                if self.universes.len() > DISCOVERY_UNI_PER_PAGE {
                    return Err(SacnError::SacnParsePackError(
                        sacn_parse_pack_error::SacnParsePackError::PackInvalidData(
                            format!("Maximum {} universes allowed per discovery page", DISCOVERY_UNI_PER_PAGE).to_string())));
                }

                if buf.len() < self.len() {
                    return Err(SacnError::SacnParsePackError(
            sacn_parse_pack_error::SacnParsePackError::PackBufferInsufficient("UniverseDiscoveryPacketUniverseDiscoveryLayer pack buffer insufficient".to_string())));
                }
                ...
                for i in 1..self.universes.len() {
                    if self.universes[i] == self.universes[i - 1] {
                        return Err(SacnError::SacnParsePackError(
            sacn_parse_pack_error::SacnParsePackError::PackInvalidData("Universes are not unique".to_string())));
                    }
                    if self.universes[i] <= self.universes[i - 1] {
                        return Err(SacnError::SacnParsePackError(
            sacn_parse_pack_error::SacnParsePackError::PackInvalidData("Universes are not sorted".to_string())));
                    }
                }
...
fn parse_universe_list<'a>(buf: &[u8], length: usize) -> Result<Cow<'a, [u16]>> {
    ...
    while i < (length * E131_UNIVERSE_FIELD_LENGTH) {
        ...
        if (u as i32) > last_universe {
            ...
            return Err(SacnError::SacnParsePackError(
                            sacn_parse_pack_error::SacnParsePackError::ParseInvalidUniverseOrder(
