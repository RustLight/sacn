...
impl SacnSourceInternal {
    ...
    fn deregister_universe(&mut self, universe: u16) -> Result<()> {
        ...
        match self.universes.binary_search(&universe) {
            Err(_i) => {
                // Value not found
                Err(SacnError::UniverseNotFound(
    ...
    fn send(
        ...
        if self.running == false {
            ...
            return Err(SacnError::SenderAlreadyTerminated("Attempted to send".to_string()));
        }
        ...
        if synchronisation_addr.is_some() {
            self.universe_allowed(&synchronisation_addr.unwrap())
                .map_err(|e| SacnError::UniverseNotRegistered(format!(
                        "Synchronisation universe not allowed: {e}"
                    )))?;
        }
    ...
    fn send_universe(
        ...
        if priority > E131_MAX_PRIORITY {
            return Err(SacnError::InvalidPriority(format!(
        ...
        if data.len() > UNIVERSE_CHANNEL_CAPACITY {
            return Err(SacnError::ExceedUniverseCapacity(format!(
        ...
        if dst_ip.is_some() {
            self.socket
                ...
                .with_context(|| "Failed to send data unicast on socket".to_string())?;
        } else {
            let dst;

            if self.addr.is_ipv6() {
                dst = universe_to_ipv6_multicast_addr(universe)
                    .with_context(|| "Failed to convert universe to Ipv6 multicast address".to_string())?;
            } else {
                dst = universe_to_ipv4_multicast_addr(universe)
                    .with_context(|| "Failed to convert universe to Ipv4 multicast address".to_string())?;
            }

            self.socket
                .send_to(&packet.pack_alloc().unwrap(), &dst)
                .with_context(|| "Failed to send data multicast on socket".to_string())?;
        }
    ...
    fn send_sync_packet(&self, universe: u16, dst_ip: Option<SocketAddr>) -> Result<()> {
        ...
        self.socket
            ...
            .with_context(|| "Failed to send sync packet on socket".to_string())?;
