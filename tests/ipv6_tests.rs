...
mod sacn_ipv6_multicast_test {
    ...
    use sacn::packet::{
        ACN_SDT_MULTICAST_PORT, E131_DISCOVERY_UNIVERSE, E131_TERMINATE_STREAM_PACKET_COUNT,
        UNIVERSE_CHANNEL_CAPACITY, universe_to_ipv4_multicast_addr,
    ...
    fn test_send_recv_partial_capacity_universe_multicast_ipv6() {
        let (tx, rx): (Sender<Result<Vec<DMXData>>>, Receiver<Result<Vec<DMXData>>>) =
            mpsc::channel();
    ...
    fn test_three_senders_three_recv_multicast_ipv6() {
        ...
        let (rcv_tx, rcv_rx): (
            SyncSender<Vec<Result<Vec<DMXData>>>>,
            Receiver<Vec<Result<Vec<DMXData>>>>,
        ) = mpsc::sync_channel(0);
    ...
    fn test_ip_equivalence() {
        ...
        let mut ipv4_source =
            SacnSource::with_cid_ip(&source_name.clone(), Uuid::from_bytes(CID), ipv4).unwrap();
        ...
        let mut ipv6_source =
            SacnSource::with_cid_ip(&source_name.clone(), Uuid::from_bytes(CID), ipv6).unwrap();
