...
Ipv4 testing requires that the computer have (a) network interface(s) with the IPs of `192.168.1.6`, `192.168.1.7`, and `192.168.1.8`. These IPs are set in the testing file. Ip tests are ignored by default. Run the ip tests in a single thread to avoid socket conflicts within the OS. To run ipv4 tests, use
